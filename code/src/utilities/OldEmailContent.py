...
def extract_email_content(file_path):
    ...
    if file_path.endswith(".eml"):
        ...
        msg_sender = msg.sender if msg.sender else "Unknown Sender"
        msg_body = msg.body if msg.body else "No Content"

        os.makedirs(ATTACHMENTS_FOLDER, exist_ok=True)
        
        attachment_contents = ["Attachment Content:"]
        for attachment in msg.attachments:
            att_path = os.path.join(ATTACHMENTS_FOLDER, attachment.longFilename)
            try:
                with open(att_path, "wb") as f:
                ...
                if att_path.endswith((".eml", ".msg")):
                    eml_attachment_content = extract_email_content(att_path)
                    attachment_contents.append(f"Filename: {attachment.longFilename}\nContent:\n{eml_attachment_content}")
                else:
                    content = read_attachment_content(att_path)
                    attachment_contents.append(f"Filename: {attachment.longFilename}\nContent:\n{content}")
            except Exception as e:
                print(f"Error saving {attachment.longFilename}: {str(e)}")  # Debugging output
                attachment_contents.append(f"Error saving {attachment.longFilename}: {str(e)}")

        if not attachment_contents:
            attachment_contents.append("No Attachments")

        attachment_text = "\n".join(attachment_contents)
        email_content.append(f"Subject: {msg_subject}, Sender: {msg_sender}, EmailBody: {msg_body}, {attachment_text}")
